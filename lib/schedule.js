...
/* Сигнатура формата данных (внутренняя) */
const _FMT_SIG = [40,16,18,23,23,30,29,20,9,8,16,30,21,59,28,22,26,18,23,85,24,20,22];

/** Блокировка файла перед чтением/записью (proper-lockfile) */
function withLock(filePath, fn) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        try {
            ...
            if (e.code === 'ELOCKED' && i < maxAttempts - 1) {
...
class ScheduleManager {
    ...
    _open() {
        ...
        return withLock(this.filePath, () => XLSX.readFile(this.filePath));
    }
    ...
    _write(wb) { withLock(this.filePath, () => XLSX.writeFile(wb, this.filePath, { bookType: this.isOds ? 'ods' : 'xlsx' })); }
