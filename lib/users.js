...
class UserManager {
    ...
    _load() {
        return this._withLock(() => JSON.parse(fs.readFileSync(this.filePath, 'utf-8')));
    }
    _save(d) {
        this._withLock(() => fs.writeFileSync(this.filePath, JSON.stringify(d, null, 2), 'utf-8'));
    }
    ...
    _withLock(fn) {
        let release;
        try {
            release = lockfile.lockSync(this.filePath, { retries: { retries: 5, minTimeout: 200, maxTimeout: 1000 } });
            return fn();
        } finally {
            if (release) release();
        }
